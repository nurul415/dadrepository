...
public class ServerParcelFrame extends JFrame
{
	...
	private JTextArea parcelRequestStatus;
	...
	public ServerParcelFrame () 
	{
		...
		// Row, Column
		this.parcelRequestStatus  = new JTextArea(20, 70);
	...
	private JPanel getRequestStatusPanel () 
	{
		...
		parcelRequestStatus.setText("\n > Server is running");
		parcelRequestStatus.setEditable(false);
		
		// Styling the request text
		parcelRequestStatus.setFont(new Font("Courier", Font.PLAIN, 15));


		// Add component to panel
		panel.add(parcelRequestStatus);
	...
	public void updateRequestStatus (String status) {
		...
		String currentText = this.parcelRequestStatus.getText();
		parcelRequestStatus.setEditable(true);
		
		// Display the latest status on top
		status += "\n > " + currentText;
		this.parcelRequestStatus.setText(status);
		parcelRequestStatus.setEditable(false);
	}
